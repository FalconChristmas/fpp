...
void FalconSetData(int sock, struct sockaddr_in* srcAddr, unsigned char* inBuf) {
    ...
    case 0xCC:
        strcpy(filename, "Falcon.FPD");
        len = 1024;
        break;
    case 0xCD:
        strcpy(filename, "Falcon.F16V2-alpha");
        len = 1024;
        break;
    }
...
int DetectFalconHardware(int configureHardware) {
    ...
    if ((responseSize == FALCON_CFG_BUF_SIZE) && (query[0] > 0)) {
        ...
        case 0x01:
            strcpy(model, "F16 v2.x");
            strcpy(cfgFile, "Falcon.F16V2-alpha");
            spiSpeed = 16000000;
            break;
        case 0x02:
            strcpy(model, "FPD v1.x");
            strcpy(cfgFile, "Falcon.FPDV1");
            spiSpeed = 8000000;
            break;
        case 0x03:
            strcpy(model, "FPD v2.x");
        ...
        case 0x81:
            strcpy(model, "Ron's Holiday Lights - FPGA Pi Pixel");
            strcpy(cfgFile, "");
            spiSpeed = 16000000;
            break;
        }
