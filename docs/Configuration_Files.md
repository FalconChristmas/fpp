...
## Plugins To Reinstall After An FPP OS Upgrade

```
pluginReinstallNeededAfterOS = "fpp-BigButtons,fpp-HomeAssistant"
```

A line in the settings file. Boot sets it to `1` after an FPP OS reflash when
plugins are installed; the Plugin Manager turns that into the names of every
plugin installed at that moment, removes each one as it is reinstalled,
updated or uninstalled, and clears the line when none is left. fppd shows the
