...
function help_help()
{
    $endpoints = array(
        ...
        ['POST /playlists', 'Insert a new playlist.', '{ "name": "UploadTest", "globalPauseBetweenSequencesMS": 5000, "mainPlaylist": [ { "type": "pause", "enabled": 1, "playOnce": 0, "duration": 8 } ], "playlistInfo": { "total_duration": 8, "total_items": 1 } }', '{ "Status": "OK", "Message": "" }'],
        ['GET /playlist/:PlaylistName', 'Get Playlist named :PlaylistName.  Returns a normal FPP playlist in JSON format.', '', '{ "name": "UploadTest", "globalPauseBetweenSequencesMS": 5000, "mainPlaylist": [ { "type": "pause", "enabled": 1, "playOnce": 0, "duration": 8 } ], "playlistInfo": { "total_duration": 8, "total_items": 1 } }'],
        ['GET /playlist/:PlaylistName/start', 'Start the playlist named :PlaylistName.', '', ''],
        ['GET /playlist/repeat/:Repeat', 'Turn repeat on or off for the running playlist without starting, stopping or restarting it.  :Repeat is true/false or 1/0.  Returns an error if no playlist is running.', '', '{ "Status": "OK", "Message": "Repeat Enabled" }'],
        ['POST /playlist/:PlaylistName', 'Update/Insert the Playlist named :PlaylistName.', '{ "name": "UploadTest", "globalPauseBetweenSequencesMS": 5000, "mainPlaylist": [ { "type": "pause", "enabled": 1, "playOnce": 0, "duration": 8 } ], "playlistInfo": { "total_duration": 8, "total_items": 1 } }', '{ "Status": "OK", "Message": "" }'],
...
";
    ...
    foreach ($endpoints as $endpoint) {
        ...
        if ((preg_match('/^GET \//', $endpoint[0])) && (!preg_match('/:/', $endpoint[0]))) {
            $desc = sprintf(
                "<a href='/api/%s' target='_blank'>%s</a>",
                preg_replace('/^GET \//', '', $endpoint[0]),
                $endpoint[0]
            );
        }

        $h .= sprintf(
            "<tr><td class='endpoint'>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>\n",
            $desc,
            $endpoint[1],
            $input,
            $output
