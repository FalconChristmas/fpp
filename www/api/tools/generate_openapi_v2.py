#!/usr/bin/env python3
"""
Generate www/api/v2/openapi.json from @route/@body/@response PHPDoc tags
...
OUTPUT = Path(__file__).parent.parent / 'v2' / 'openapi.json'
...
def load_endpoints():
    ...
    for php_file in CONTROLLERS:
        ...
        for ep in parse_docblocks(source):
            endpoints.append(ep)
    endpoints = rewrite_v2_paths(endpoints)
    endpoints.sort(key=lambda e: (e['path'], e['method']))
    return endpoints


def rewrite_v2_paths(endpoints):
    rewritten = []
    for ep in endpoints:
        current = dict(ep)
        if current['path'] in ROUTELESS_DOC_PATHS:
            continue
        if current['path'] == '/api/remoteAction' and current['method'] == 'get':
            continue
        if current['path'].startswith('/api/v2/'):
            rewritten.append(current)
            continue
        if current['path'].startswith('/api/'):
            current['path'] = current['path'].replace('/api/', '/api/v2/', 1)
            rewritten.append(current)
    return rewritten
...
def tag_from_path(path):
    parts = [p for p in path.strip('/').split('/') if p and not p.startswith('{') and p not in ('api', 'v2')]
...
def build_openapi(endpoints):
    ...
    spec = {
        ...
        'servers': [{'url': '/api/v2', 'description': 'Local FPP instance (v2)'}],
