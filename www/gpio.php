...
foreach ($gpiojson as $gpio) {
    $pinName = $gpio['pin'];
    if (isset($usedGpioPins[$pinName])) continue;
    if (count($includeFilters) > 0) {
        $matched = false;
        foreach ($includeFilters as $f) {
            if (preg_match("/" . $f . "/", $pinName)) { $matched = true; break; }
        }
        if (!$matched) continue;
    }
    $skip = false;
    foreach ($excludeFilters as $f) {
        if (preg_match("/" . $f . "/", $pinName)) { $skip = true; break; }
    }
    if (!$skip) $availablePins[] = $gpio;
}
?>
...
.gpio-cmd-list li:last-child { border-bottom: none; }
.gpio-cmd-badge {
    flex: 1;
...
.gpio-section-label:first-child { margin-top: 0; }
.gpio-empty-text {
    color: var(--fpp-text-muted);
    font-style: italic;
    font-size: var(--fpp-fs-sm);
...
$usedOut = array();
foreach ($usedGpioPins as $pin => $usage) { $usedOut[$pin] = $usage; }
...
<body>
    <div id="bodyWrapper">
        ...
        <!-- ═══ GPIO Edit Modal ═══════════════════════════════════════════════ -->
        <div class="modal fade" id="gpioEditModal" tabindex="-1" aria-labelledby="gpioEditModalLabel" aria-hidden="true">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="gpio-modal-section">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input class="form-control" type="text" id="gpioModalDesc" maxlength="128" placeholder="e.g. Start button">
                                </div>
                                <div class="col-md-4">
                            ...
                            <div class="row g-3 mt-1">
                                <div class="col-md-3">
                                    ...
                                    <input class="form-control" type="number" id="gpioModalDebounce" min="10" max="60000" value="100">
                                </div>
                                <div class="col-md-3">
                        ...
                        <div class="gpio-modal-section">
                            ...
                            <div id="gpioLEDSettingsRow" style="display:none" class="mt-3">
                                ...
                                <hr class="my-3">
                                <h6 class="fw-semibold mb-2"><i class="fas fa-moon text-secondary"></i> Idle / Standby State</h6>
                                ...
                                <div class="row g-3">
                                    <div class="col-md-4">
                                        ...
                                        <select class="form-select" id="gpioModalLedIdleMode" onchange="updateLEDVisibility()">
                                    ...
                                    <div class="col-md-4" id="gpioLEDPulseRateRow" style="display:none">
                                        ...
                                        <input class="form-control" type="number" id="gpioModalLedPulseRate" min="50" max="5000" value="500">
                                        <div class="form-text">Time between LED on↔off toggles (e.g. 500 = 1 Hz blink).</div>
                                    </div>
                                </div>

                                <hr class="my-3">
                                <h6 class="fw-semibold mb-2"><i class="fas fa-bolt text-warning"></i> Trigger Effect</h6>
                                <p class="small text-muted mb-2">What the LED does when the button fires (rising edge).</p>
                                <div class="row g-3">
                                    <div class="col-md-5">
                                        <label class="form-label fw-semibold">Trigger Mode</label>
                                        <select class="form-select" id="gpioModalLedTriggerMode" onchange="updateLEDVisibility()">
                                    ...
                                    <div class="col-md-3" id="gpioLEDTriggerParamRow" style="display:none">
                                        <label class="form-label fw-semibold" id="gpioLEDTriggerParamLabel">Number of flashes</label>
                                        <input class="form-control" type="number" id="gpioModalLedTriggerParam" min="1" max="20" value="3">
