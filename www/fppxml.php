...
if (isset($_GET['command']) && !empty($_GET['command'])) {
    ...
    return;
} elseif (!empty($_POST['command']) && $_POST['command'] == "saveUniverses") {
    SetUniverses();
} elseif (!empty($_POST['command']) && $_POST['command'] == "saveSchedule") {
    SaveSchedule();
} elseif (!empty($_POST['command']) && $_POST['command'] == "saveHardwareConfig") {
    SaveHardwareConfig();
}
...
function MoveFile()
{
    ...
    if (file_exists($uploadDirectory."/" . $file)) {
        ...
        } elseif (preg_match("/\.(mp4|mkv)$/i", $file)) {
        ...
        } elseif (preg_match("/\.(mp3|ogg)$/i", $file)) {
...
function GetFiles()
{
    ...
    } elseif ($dirName == "Music") {
        $dirName = $musicDirectory;
    } elseif ($dirName == "Videos") {
        $dirName = $videoDirectory;
    } elseif ($dirName == "Effects") {
        $dirName = $effectDirectory;
    } elseif ($dirName == "Scripts") {
        $dirName = $scriptDirectory;
    } elseif ($dirName == "Logs") {
        $dirName = $logDirectory;
    } elseif ($dirName == "Uploads") {
...
function SavePlaylist()
{
    ...
    for ($i=0;$i<count($_SESSION['playListEntries']);$i++) {
        ...
        } elseif ($_SESSION['playListEntries'][$i]->type == 's') {
            $entries .= sprintf("%s,%s,\n", $_SESSION['playListEntries'][$i]->type, $_SESSION['playListEntries'][$i]->seqFile);
        } elseif ($_SESSION['playListEntries'][$i]->type == 'm') {
            $entries .= sprintf("%s,%s,\n", $_SESSION['playListEntries'][$i]->type, $_SESSION['playListEntries'][$i]->songFile);
        } elseif ($_SESSION['playListEntries'][$i]->type == 'p') {
            $entries .= sprintf("%s,%s,\n", $_SESSION['playListEntries'][$i]->type, $_SESSION['playListEntries'][$i]->pause);
        } elseif ($_SESSION['playListEntries'][$i]->type == 'e') {
...
function GetFile()
{
    ...
    if (isset($_GET['play'])) {
        ...
        } elseif (preg_match('/ogg$/i', $filename)) {
