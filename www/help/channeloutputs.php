...
<ul>
    <li><b>Tabs</b> — E1.31/DDP, Strings, PWM and LED Panels each show only outputs of that family. The active tab is highlighted; the content below matches the tab. The page opens on the tab that matches the detected cape — a pixel cape opens on its Pixel Strings tab, a panel-only cape (OctoPlus, PocketScroller) on its LED Panels tab. With no cape detected, it opens on E1.31/DDP. Tabs your hardware cannot drive are hidden. The E1.31/DDP tab can also be hidden deliberately — see <b>Hide Network Output Tab</b> under Settings &gt; User Interface. That setting is ignored while a universe output is enabled, so the tab comes back on its own whenever one is configured, and <b>Display all hardware options/settings</b> brings it back regardless. Hiding the tab only hides it; it never disables an output that is already configured.</li>
