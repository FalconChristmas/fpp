...
class fppLCD():
  ...
  def __init__(self):
    ...
    self.MENU_INX_START_PLAYLIST = 2
    self.MENU_INX__SHUTDOWN_REBOOT = 3

    self.MENU_INX_BACKCOLOR = 10
  ...
  def ProcessSelect(self):
    ...
    if self.MenuIndex == self.MENU_INX_STATUS:
      if self.FPPDstatus == self.FPPD_STATUS_STOPPED:
        ...
        self.MenuIndex = self.MENU_INX_START_PLAYLIST
        self.SubmenuIndex = 0
        self.DisplayStartPlaylist()
  ...
  def SelectColor(self):
    self.lcd.backlight(self.colors[self.SubmenuIndex][1])
  ...
  def DisplayStartPlaylist(self):
    self.line1 = self.MakeStringWithLength("Start Playlist",16,0)
    if self.SubmenuIndex == 0:
    ...
    self.line2 = self.MakeStringWithLength(str(self.SubmenuIndex+1) + ". " + str(self.playlists[self.SubmenuIndex]),16,0)
    self.UpdateDisplay()
    return
  ...
  def StartPlaylist(self):
    self.SetStatusTimeOut(self.NORMAL_STATUS_TIMEOUT)
    if self.SubmenuIndex < len(self.playlists):
      command = "p," + str(self.playlists[self.SubmenuIndex]) + ",0"
